...
ESP_P47_water_plot_n   <- ggplot(ESP_P47_water, aes(x = Q, y = n,shape = RPM,colour=fluid))   + geom_point() + theme_bw()   + ggtitle ("Flow rate Q (m3/h) vs. Efficiency n (%)") + ylab("Efficiency n (%)")  +  xlab("Flow rate Q m3/h")+ theme(legend.position = "bottom")

# Melt tabele

# Plot_raw_vibration_data.png                                                                                                            
png(filename=paste(project_folder,"ESP_P47_water_RPM3500.png",sep=""), width = 15, height = 20, res=600, units = "cm")  
